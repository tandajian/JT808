...
7E                  --头标识                 1 字节
02 00               --数据头->消息ID      　 2 字节
00 26               --数据头->消息体属性      2 字节
12 34 56 78 90 12   --数据头->终端手机号      6 字节
00 7E               --数据头->消息流水号      2 字节
00 00 00 01         --消息体->报警标志        4 字节
00 00 00 02         --消息体->状态位标志      4 字节
00 BA 7F 0E         --消息体->纬度           4 字节
07 E4 F1 1C         --消息体->经度           4 字节
00 28               --消息体->海拔高度        2 字节
00 3C               --消息体->速度           2 字节
00 00               --消息体->方向           2 字节
18 10 15 10 10 10   --消息体->GPS时间        6 字节
01                  --消息体->附加信息->里程
04                  --消息体->附加信息->长度
