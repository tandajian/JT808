...
namespace JT808.Protocol.Formatters
{
    ...
    public class JT808PackageFormatter : IJT808Formatter<JT808Package>
    {
        public JT808Package Deserialize(ReadOnlySpan<byte> bytes, out int readSize)
        {
            ...
            try
            {
                jT808Package.Header = JT808FormatterExtensions.HeaderFormatter.Deserialize(buffer.Slice(offset), out readSize);
            }
            catch (Exception ex)
            {
                ...
                if (jT808BodiesImplType != null)
                {
                    ...
                    if (jT808Package.Header.MessageBodyProperty.IsPackge)
                    {
                        ...
                        if (jT808Package.Header.MessageBodyProperty.PackageIndex > 1)
                        {
                            ...
                            {
                                //5.处理第二包之后的分包数据消息体
                                jT808Package.Bodies = JT808FormatterExtensions.SplitPackageBodiesFormatter.Deserialize(buffer.Slice(offset + 1, jT808Package.Header.MessageBodyProperty.DataLength),out readSize);
                            }
                            catch (Exception ex)
                            {
        ...
        public int Serialize(ref byte[] bytes, int offset, JT808Package value)
        {
            ...
            offset = JT808FormatterExtensions.HeaderFormatter.Serialize(ref bytes, offset, value.Header);
            if (messageBodyOffset != 0)
            {
