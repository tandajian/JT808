    ...
    public class JT808HeaderFormatter : IJT808Formatter<JT808Header>
    {
        public JT808Header Deserialize(ReadOnlySpan<byte> bytes, out int readSize)
        {
            ...
            JT808Header jT808Header = new JT808Header
            {
                ...
                // 2.消息体属性
                MessageBodyProperty = JT808FormatterExtensions.HeaderMessageBodyPropertyFormatter.Deserialize(bytes.Slice(offset), out readSize)
            };
            offset += readSize;
            // 3.终端手机号 (写死大陆手机号码)
        ...
        public int Serialize(ref byte[] bytes, int offset, JT808Header value)
        {
            ...
            //2.消息体属性
            offset = JT808FormatterExtensions.HeaderMessageBodyPropertyFormatter.Serialize(ref bytes, offset, value.MessageBodyProperty);
            // 3.终端手机号 (写死大陆手机号码)
